...
export function CultureCenter({ initialData }: { initialData: CultureData }) {
  ...
  const canEdit = !!config && !data.warning && data.channels.length > 0
  ...
  return <main className={styles.root}>
    <div className={styles.frame}>
      <header className={styles.hero}>
        <div><p className={styles.eyebrow}>KIT / STUDIO CULTURE <span>ADMIN ONLY</span></p><h1>Culture<br /><span>Center.</span></h1><p className={styles.description}>The rituals that make the studio.<br />One place to manage memes, celebrations and good timing.</p></div>
        <div className={styles.heroActions}><button className={styles.primary} disabled={!canEdit} onClick={() => setEditing(newMeme('custom', config!.default_channel_id, config!.timezone, crypto.randomUUID()))}>+ Add meme</button><button disabled={refreshing} onClick={() => void refresh()}>{refreshing ? 'Refreshing…' : 'Refresh'}</button><small>Snapshot {timestamp(data.generatedAt, config?.timezone)}<br />Refresh to see the latest worker activity.</small></div>
      </header>
      <div className={styles.metrics}><div><strong>{String(enabled).padStart(2, '0')}</strong><span>Enabled rules</span></div><div><strong>{String(drafts).padStart(2, '0')}</strong><span>Drafts to shape</span></div><div><strong>{String(review).padStart(2, '0')}</strong><span>Posts needing review*</span></div><div><strong className={fresh ? styles.green : ''}>{!config ? 'SETUP' : pending ? 'STAGED' : fresh ? 'ONLINE' : 'CHECK'}</strong><span>{!config ? 'Import existing rituals below' : pending ? 'Scheduled handover' : fresh ? 'Worker checked in' : 'Worker not recently seen'}</span></div></div>
      ...
      {KINDS.map((kind, index) => <section className={styles.section} key={kind} aria-labelledby={`section-${kind}`}>
        <div className={styles.sectionHeading}><div><p className={styles.eyebrow}>{String(index + 1).padStart(2, '0')} / CULTURE RULES</p><h2 id={`section-${kind}`}>{SECTION_NAMES[kind]}</h2><p>{DESCRIPTIONS[kind]}</p></div><button disabled={!canEdit} aria-label={`Add ${kind} meme`} onClick={() => setEditing(newMeme(kind, config!.default_channel_id, config!.timezone, crypto.randomUUID()))}>+ Add</button></div>
      ...
      <footer className={styles.footer}>RANGERANDFOX.STUDIO <span>INTERNAL CHANNELS ONLY / PUBLIC-SAFE COPY / NO FINANCIAL OR CLIENT CONTACT DATA</span></footer>
    </div>
...
function MemeEditor({ initial, data, onClose, onSaved }: { initial: Meme; data: CultureData; onClose: () => void; onSaved: (message: string) => Promise<void> }) {
  ...
  return <dialog ref={dialog} className={styles.dialog} onCancel={event => { event.preventDefault(); if (!saving) onClose() }} aria-labelledby="editor-title">
    <form onSubmit={submit}>
      <header className={styles.dialogHeader}><div><p className={styles.eyebrow}>{isNew ? 'NEW RITUAL / DRAFT FIRST' : 'CULTURE RULE / EDIT'}</p><h2 id="editor-title">{isNew ? 'Make it a thing.' : 'Shape the ritual.'}</h2></div><button type="button" onClick={onClose} disabled={saving} aria-label="Close editor">×</button></header>
      <fieldset disabled={saving} className={styles.formGrid}>
        ...
        <label>Destination channel<select required value={item.channel_id} onChange={event => update({ channel_id: event.target.value })}><option value="">Choose a channel</option>{!data.channels.some(channel => channel.id === item.channel_id) && item.channel_id ? <option value={item.channel_id} disabled>Unavailable — choose another channel</option> : null}{data.channels.map(channel => <option key={channel.id} value={channel.id}>#{channel.name}</option>)}</select></label>
      ...
      <footer className={styles.dialogFooter}><button type="button" onClick={onClose} disabled={saving}>Cancel</button><button className={styles.primary} disabled={saving || (item.status === 'enabled' && !confirmed)}>{saving ? 'Saving…' : isNew ? 'Save draft' : 'Save changes'}</button></footer>
    </form>
  </dialog>
}
