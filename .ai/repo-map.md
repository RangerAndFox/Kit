...
| Project update ripple | Edit an established project via a pre-filled modal + preview/confirm, then ripple the change across every outlet (Slack channel rename, Harvest/Frame.io rename, Dropbox folder move, Master Project List row + Canvas, Supabase). Durable per-service step ledger (migration 063), reconcile-by-Kit-marker, Railway-recovered. Identity strings come from one shared derivation shared with create. | `/kit update` / "update project" → `bolt/src/handlers/interactions.ts` (`kit_update_project` view + `kit_update_confirm` → `runUpdateRipple`) → `src/lib/provisioner/update.ts` (`runProjectUpdate`) | `src/lib/provisioner/` + per-service agent `rename` actions | Railway (Bolt); **Supabase authoritative** | `npx tsx --test src/lib/provisioner/*.test.ts` | `provisioner/identifiers.ts` + `update-diff.ts`, then `update.ts` + `update-store.ts` | V |
...
| Culture Center | Admin meme tables, draft/edit controls, upcoming and posting history | `src/app/(app)/culture-center/page.tsx`, `src/app/api/culture-center/route.ts` | `src/lib/culture/`, `bolt/src/culture/` | Vercel controls; Railway schedules; Supabase authoritative | `npm run test:app`, Bolt vitest, `scripts/test-culture-database.mjs` | `.ai/workflows/culture-center.md`, `culture/model.ts`, `culture/http.ts`, `culture/runner.ts` | V code / ? live |
| Vercel HTTP request surface | Publicly reachable API routes. `src/proxy.ts` (Next 16 middleware — **not** `middleware.ts`) runs on every non-static path incl. `/api/*`, refreshes the Supabase session, and delegates authorization to each handler. Authorization owners and the fail-closed rule are invariant 16→17 in `.ai/invariants.md`. **Slack has no HTTP route here** — it arrives via Socket Mode on Railway | `src/proxy.ts`, then the specific `src/app/api/**/route.ts` | `src/app/api/`, `src/lib/mcp/` | Vercel | `npx tsx --test src/lib/project-control/webhook-auth.test.ts src/app/api/**/route.test.ts`; `npm run build` (route manifest) | `src/proxy.ts`, `src/lib/project-control/webhook-auth.ts`, then the route | V |
