...
| Vercel HTTP request surface | Publicly reachable API routes. `src/proxy.ts` (Next 16 middleware — **not** `middleware.ts`) runs on every non-static path incl. `/api/*`, refreshes the Supabase session, and delegates authorization to each handler. Authorization owners and the fail-closed rule are invariant 16→17 in `.ai/invariants.md` | `src/proxy.ts`, then the specific `src/app/api/**/route.ts` | `src/app/api/`, `src/lib/slack/`, `src/lib/mcp/` | Vercel | `npx tsx --test src/lib/slack/*.test.ts src/app/api/**/route.test.ts`; `npm run build` (route manifest) | `src/proxy.ts`, `src/lib/slack/verify.ts` + `deny.ts`, then the route | V |
