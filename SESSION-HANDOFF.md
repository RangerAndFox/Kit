...
Founder/admin project cleanup is also implemented through DM `delete project` or `/kit delete project`. It inventories linked outputs, requires `DELETE <project ID>`, deletes external providers before the authoritative Kit row, archives the Slack channel, and persists a retryable audit ledger. Manually linked or published resources that Kit cannot prove it owns are retained and disclosed in the confirmation modal.
