...
Six classes were requested. Result: **verified-complete 0, superseded 7, intentionally disabled/retired 36, provider-asset-missing 0 (unverifiable), genuinely-incomplete 2, unresolved/in-flight 11.** All 56 have a `frameio_file_id` recorded (asset identity present in the ledger); none is itself in `ready`.

| Class | Rows | Which | Basis |
|---|---:|---|---|
| **Verified complete** | 0 | — | No row in this set is itself `ready`. (7 are superseded by a completed sibling — next row.) |
| **Superseded** (a later `ready` transfer succeeded for the same Dropbox file) | 7 | 2625 ×2 (n2,n3); 2631 ×2 (n7,n8); 2637 ×3 (n18–20) | `ready_sib = true`: the delivery *did* complete on a subsequent transfer; these rows are redundant duplicates. |
| **Intentionally disabled** (project auto-upload OFF) | 28 | 2639 (n29–56) | `project_settings.frameio_upload_enabled = FALSE` for 2639. All `created`, no successor — consistent with upload being turned off ~23.6 d ago. **Will not re-drive by design. Preserve.** |
| **Intentionally retired by instruction** (Fabric queue) | 8 | 2637 (n21–28) | 2637 = "Microsoft Fabric IQ sizzle"; prior instruction is to retire Fabric queue items. These 8 have **no** successful sibling. **⚠ Caveat below.** |
| **Provider asset missing** | 0* | — | *Not determinable here.* All 56 carry a `frameio_file_id`, but whether each asset still exists on Frame.io needs a live GET (no connector). |
| **Genuinely incomplete** (old, upload enabled, no successful sibling, not retired) | 2 | 2625 (n1, 23.8 d); 2629 (n4, 3.0 d) | Past the 24 h window, upload **enabled**, no `ready` sibling. Real stuck candidates — **verify live before any action.** |
| **Unresolved / in-flight** (recent `pending`, upload enabled, may still be transcoding) | 11 | 2631 ×5 (n5,n6,n9–11); 2633 ×3 (n12–14); 2636 ×3 (n15–17) | < 2 days old; normal Frame.io transcode latency not yet exceeded. Expected to self-resolve to `ready` or age into "incomplete". |
...
- **2625 Azure — 3 rows.** n2,n3 (same rev `0165a5f2`) are **superseded** (ready sibling). n1 (rev `0165a5ca`, 23.8 d) has **no** ready sibling → **genuinely incomplete candidate**.
- **2629 Microsoft (MRA) — 1 row (n4, `pending`, 3.0 d).** Upload enabled, no sibling, past the 24 h window → **genuinely incomplete candidate**. 2629 is an active-line project, so do **not** assume closure.
...
3. **Live-verify the 2 genuinely-incomplete rows** (2625 n1, 2629 n4) against Frame.io before deciding retry vs. fail; 2629 especially, being an active project.
4. **Let the 11 in-flight rows resolve on their own**; re-check in a few days — any still `pending` then join the "genuinely incomplete" set.
